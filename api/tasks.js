...
export default async function handler(req, res) {
  ...
  const authed = await authenticate(req).catch(() => false);
  if (!authed) return res.status(401).json({ error: 'Unauthorized. Silakan login.' });
  ...
  try {
    ...
    if (req.method === 'POST' && url === '/api/tasks') {
      ...
      if (!body.name || !body.matkul) return res.status(400).json({ error: 'name dan matkul wajib diisi' });
      const tasks = await get(TASKS_KEY) || [];
      const task = {
    ...
    if (req.method === 'DELETE' && delMatch) {
      ...
      if (filtered.length === tasks.length) return res.status(404).json({ error: 'Task tidak ditemukan' });
      await set(TASKS_KEY, filtered);
